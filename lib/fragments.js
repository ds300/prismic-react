...
F.Text.asReact =
  ...
  function () {
    return el('span', null, this.value);
  };

F.DocumentLink.prototype.asReact = function (ctx) {
  return el('a', {href: this.url(ctx)}, this.value);
};
...
F.Date.prototype.asReact =
  ...
  function () {
    return el('time', null, this.value);
  };
...
function serialize(element, content) {
  ...
  if (TAG_NAMES[element.type]) {
    ...
    return el(name, {className: className}, content);
  }
  ...
  if (element.type === 'hyperlink') {
    return el('a', {href: element.url}, content);
  }

  if (element.type === 'label') {
    return el('span', {className: element.data.label}, content);
  }
  ...
  function _insertSpans (span) {
    ...
    for (var pos = span.start; pos < span.end; pos++) { // Looping to length + 1 to catch closing tags
      ...
      if (spans && spans.length) {
        ...
        childrens.push(_insertSpans(text, childSpan.start, childSpan.end));
